...
class ModelManager:
    """Manages multiple models with fallback support for Carbon Footprint Assistant"""
    ...
    def __init__(self):
        ...
        self.app_name = os.getenv('OPENROUTER_APP_NAME', 'CarbonFootprintAssistant')
        ...
        self.agent_models = {
            'footprint_analyzer': os.getenv('FOOTPRINT_ANALYZER_MODEL', self.primary_model),
            'reduction_strategist': os.getenv('REDUCTION_STRATEGIST_MODEL', self.primary_model),
            'personalization_agent': os.getenv('PERSONALIZATION_AGENT_MODEL', self.primary_model),
            'education_agent': os.getenv('EDUCATION_AGENT_MODEL', self.primary_model),
            'tracking_agent': os.getenv('TRACKING_AGENT_MODEL', self.primary_model)
        }
        ...
        logger.info(f"🚀 Model Manager initialized for Carbon Footprint Assistant")
    ...
    def _get_agent_temperature(self, agent_type: str) -> float:
        temperatures = {
            'footprint_analyzer': 0.2,
            'reduction_strategist': 0.4,
            'personalization_agent': 0.5,
            'education_agent': 0.6,
            'tracking_agent': 0.3
        }
